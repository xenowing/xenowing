...
fn main() {
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        let mut assemble_triangle = |mut verts: [Vertex; 3]| {
            let viewport_x = 0;
            let viewport_y = 0;
            ...
            let w_fract_bits = 8;
            device.write_reg(REG_W0_DX_ADDR, to_fixed(w0_dx, w_fract_bits) as _);
            device.write_reg(REG_W1_DX_ADDR, to_fixed(w1_dx, w_fract_bits) as _);
            device.write_reg(REG_W2_DX_ADDR, to_fixed(w2_dx, w_fract_bits) as _);
            device.write_reg(REG_W0_DY_ADDR, to_fixed(w0_dy, w_fract_bits) as _);
            device.write_reg(REG_W1_DY_ADDR, to_fixed(w1_dy, w_fract_bits) as _);
            device.write_reg(REG_W2_DY_ADDR, to_fixed(w2_dy, w_fract_bits) as _);
            ...
            device.write_reg(REG_R_DX_ADDR, to_fixed(r_dx, color_fract_bits) as _);
            device.write_reg(REG_G_DX_ADDR, to_fixed(g_dx, color_fract_bits) as _);
            device.write_reg(REG_B_DX_ADDR, to_fixed(b_dx, color_fract_bits) as _);
            device.write_reg(REG_A_DX_ADDR, to_fixed(a_dx, color_fract_bits) as _);
            device.write_reg(REG_R_DY_ADDR, to_fixed(r_dy, color_fract_bits) as _);
            device.write_reg(REG_G_DY_ADDR, to_fixed(g_dy, color_fract_bits) as _);
            device.write_reg(REG_B_DY_ADDR, to_fixed(b_dy, color_fract_bits) as _);
            device.write_reg(REG_A_DY_ADDR, to_fixed(a_dy, color_fract_bits) as _);
            ...
            device.write_reg(REG_W_INVERSE_DX_ADDR, to_fixed(w_inverse_dx, W_INVERSE_FRACT_BITS) as _);
            device.write_reg(REG_W_INVERSE_DY_ADDR, to_fixed(w_inverse_dy, W_INVERSE_FRACT_BITS) as _);
            ...
            device.write_reg(REG_S_DX_ADDR, to_fixed(s_dx, ST_FRACT_BITS) as _);
            device.write_reg(REG_T_DX_ADDR, to_fixed(t_dx, ST_FRACT_BITS) as _);
            device.write_reg(REG_S_DY_ADDR, to_fixed(s_dy, ST_FRACT_BITS) as _);
            device.write_reg(REG_T_DY_ADDR, to_fixed(t_dy, ST_FRACT_BITS) as _);
                ...
                for tile_index_x in 0..WIDTH / (TILE_DIM as usize) {
                    ...
                    for y in 0..TILE_DIM as usize {
                        for x in 0..TILE_DIM as usize {
                            ...
                            device.write_color_buffer_word(y as u32 * TILE_DIM + x as u32, back_buffer[buffer_index]);
                        }
                    }
                    ...
                    device.write_reg(REG_W0_MIN_ADDR, to_fixed(w0_min, w_fract_bits) as _);
                    device.write_reg(REG_W1_MIN_ADDR, to_fixed(w1_min, w_fract_bits) as _);
                    device.write_reg(REG_W2_MIN_ADDR, to_fixed(w2_min, w_fract_bits) as _);
                    ...
                    device.write_reg(REG_R_MIN_ADDR, to_fixed(r_min, color_fract_bits) as _);
                    device.write_reg(REG_G_MIN_ADDR, to_fixed(g_min, color_fract_bits) as _);
                    device.write_reg(REG_B_MIN_ADDR, to_fixed(b_min, color_fract_bits) as _);
                    device.write_reg(REG_A_MIN_ADDR, to_fixed(a_min, color_fract_bits) as _);

                    let w_inverse_min = 1.0 / verts[0].position.w() * w0_min + 1.0 / verts[1].position.w() * w1_min + 1.0 / verts[2].position.w() * w2_min;
                    device.write_reg(REG_W_INVERSE_MIN_ADDR, to_fixed(w_inverse_min, W_INVERSE_FRACT_BITS) as _);
                    ...
                    device.write_reg(REG_S_MIN_ADDR, to_fixed(s_min, ST_FRACT_BITS) as _);
                    device.write_reg(REG_T_MIN_ADDR, to_fixed(t_min, ST_FRACT_BITS) as _);
                    ...
                    for y in 0..TILE_DIM as usize {
                        for x in 0..TILE_DIM as usize {
                            let buffer_index = (HEIGHT - 1 - (tile_min_y as usize + y)) * WIDTH + tile_min_x as usize + x;
                            back_buffer[buffer_index] = device.read_color_buffer_word(y as u32 * TILE_DIM + x as u32);
                        }
                    }
                }
            }
        ...
        let projection = Matrix::perspective(90.0, WIDTH as f32 / HEIGHT as f32, 1.0, 1000.0);
        ...
        verts.push(Vertex {
            ...
            color: Vec4::new(1.0, 1.0, 1.0, 1.0) * 255.0,
            tex_coord: Vec2::new(0.0, 0.0),
        });
        verts.push(Vertex {
            position: Vec4::new(1.0, -1.0, 1.0, 1.0),
            color: Vec4::new(1.0, 0.0, 0.0, 1.0) * 255.0,
            tex_coord: Vec2::new(1.0, 0.0),
        });
        verts.push(Vertex {
            position: Vec4::new(1.0, 1.0, 1.0, 1.0),
            color: Vec4::new(0.0, 1.0, 0.0, 1.0) * 255.0,
        ...
        verts.push(Vertex {
            position: Vec4::new(-1.0, 1.0, 1.0, 1.0),
            color: Vec4::new(0.0, 0.0, 1.0, 1.0) * 255.0,
        ...
        for i in (0..verts.len()).step_by(3) {
            assemble_triangle([verts[i + 0], verts[i + 1], verts[i + 2]])
        }

        /*let uv_offset = (frame as f32 * 0.002).sin();

        assemble_triangle([
            Vertex {
                position: Vec4::new(-1.0, -1.0, 0.0, 1.0),
                color: Vec4::new(1.0, 1.0, 1.0, 1.0) * 255.0,
                tex_coord: (Vec2::new(0.0, 0.0) + uv_offset) / 1.0,
            },
            Vertex {
                position: Vec4::new(1.0, -1.0, 0.0, 1.0),
                color: Vec4::new(1.0, 1.0, 1.0, 1.0) * 255.0,
                tex_coord: (Vec2::new(1.0, 0.0) + uv_offset) / 1.0,
            },
            Vertex {
                position: Vec4::new(1.0, 1.0, 0.0, 1.0),
                color: Vec4::new(1.0, 1.0, 1.0, 1.0) * 255.0,
                tex_coord: (Vec2::new(1.0, 1.0) + uv_offset) / 1.0,
            },
        ]);
        assemble_triangle([
            Vertex {
            ...
            Vertex {
                position: Vec4::new(-1.0, 1.0, 0.0, 1.0),
                color: Vec4::new(1.0, 1.0, 1.0, 1.0) * 255.0,
                tex_coord: (Vec2::new(0.0, 1.0) + uv_offset) / 1.0,
            },
            Vertex {
                position: Vec4::new(-1.0, -1.0, 0.0, 1.0),
                color: Vec4::new(1.0, 1.0, 1.0, 1.0) * 255.0,
        ...
        /*assemble_triangle([
            Vertex {
                position: Vec4::new(rand::random::<f32>() * 2.0 - 1.0, rand::random::<f32>() * 2.0 - 1.0, 0.0, 1.0),
