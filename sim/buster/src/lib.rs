...
mod tests {
    ...
    fn read_all_master1() {
        ...
        loop {
            ...
            if m.master1_bus_read_data_valid {
                master_read_data.push(m.master1_bus_read_data);
                if master_read_data.len() == data.len() {
                    assert_eq!(master_read_data, data);
                    return;
                }
            ...
            if master_read_addr < data.len() {
                m.master1_bus_enable = true;
                m.master1_bus_addr = master_read_addr as _;
            } else {
                m.master1_bus_enable = false;
            }
            m.master0_bus_enable = false;
            ...
            if m.master1_bus_enable && m.master1_bus_ready {
                master_read_addr += 1;
            }
