...
impl<'a> MarvSystemBridge<'a> {
    pub fn new(instance_name: impl Into<String>, p: &'a impl ModuleParent<'a>) -> MarvSystemBridge<'a> {
        ...
        let system_bus_addr = m.output("system_bus_addr", marv_bus_addr.bits(marv_addr_bit_width - 1, 2));
        let system_bus_write = m.output("system_bus_write", marv_bus_write);

        let marv_issue_word_select = marv_bus_addr.bits(1, 0);
        let (system_bus_write_data, system_bus_write_byte_enable) = if_(marv_issue_word_select.eq(m.lit(0b00u32, 2)), {
        ...
        let marv_bus_read_data = m.output("marv_bus_read_data", if_(read_data_word_select.eq(m.lit(0b00u32, 2)), {
            ...
            system_bus_read_data.bits(127, 96)
        }));
        let system_bus_read_data_valid = m.input("system_bus_read_data_valid", 1);
        let marv_bus_read_data_valid = m.output("marv_bus_read_data_valid", system_bus_read_data_valid);
        ...
        MarvSystemBridge {
            ...
            system_port: PrimaryPort {
                ...
                bus_write_byte_enable: m.output("system_bus_write_byte_enable", system_bus_write_byte_enable),
