...
impl<'a> Crossbar<'a> {
    pub fn new(
        ...
        let primary_issues = (0..num_primaries).map(|i| {
            ...
            let bus_addr_issue_fifo = Fifo::new(format!("{}_bus_addr_issue_fifo", name), issue_fifo_depth_bits, addr_bit_width, m);
            let bus_addr_issue_buffer = PeekBuffer::new(format!("{}_bus_addr_issue_buffer", name), addr_bit_width, m);
            ...
            let internal_handshake = bus_addr_issue_buffer.egress_ready & issue_arbiter_primary_issue.bus_ready;

            bus_addr_issue_fifo.write_data.drive(ret.bus_addr);
            bus_addr_issue_fifo.write_enable.drive(external_handshake);
            bus_addr_issue_buffer.ingress_data.drive(bus_addr_issue_fifo.read_data);
            bus_addr_issue_fifo.read_enable.drive(bus_addr_issue_buffer.ingress_read_enable);
            bus_addr_issue_buffer.ingress_data_valid.drive(
                (!bus_addr_issue_fifo.empty & bus_addr_issue_buffer.ingress_read_enable)
                .reg_next_with_default(format!("{}_bus_addr_issue_buffer_read_data_valid", name), false));
            issue_arbiter_primary_issue.bus_enable.drive(bus_addr_issue_buffer.egress_ready);
            issue_arbiter_primary_issue.bus_addr.drive(bus_addr_issue_buffer.egress_data);
            bus_addr_issue_buffer.egress_read_enable.drive(internal_handshake);

            let bus_write_issue_fifo = Fifo::new(format!("{}_bus_write_issue_fifo", name), issue_fifo_depth_bits, 1, m);
            bus_write_issue_fifo.write_data.drive(ret.bus_write);
            bus_write_issue_fifo.write_enable.drive(external_handshake);
            let bus_write_issue_buffer = PeekBuffer::new(format!("{}_bus_write_issue_buffer", name), 1, m);
            bus_write_issue_buffer.ingress_data.drive(bus_write_issue_fifo.read_data);
            bus_write_issue_fifo.read_enable.drive(bus_write_issue_buffer.ingress_read_enable);
            bus_write_issue_buffer.ingress_data_valid.drive(
                (!bus_write_issue_fifo.empty & bus_write_issue_buffer.ingress_read_enable)
                .reg_next_with_default(format!("{}_bus_write_issue_buffer_read_data_valid", name), false));
            issue_arbiter_primary_issue.bus_write.drive(bus_write_issue_buffer.egress_data);
            bus_write_issue_buffer.egress_read_enable.drive(internal_handshake);

            let bus_write_data_issue_fifo = Fifo::new(format!("{}_bus_write_data_issue_fifo", name), issue_fifo_depth_bits, data_bit_width, m);
            bus_write_data_issue_fifo.write_data.drive(ret.bus_write_data);
            bus_write_data_issue_fifo.write_enable.drive(external_handshake);
            let bus_write_data_issue_buffer = PeekBuffer::new(format!("{}_bus_write_data_issue_buffer", name), data_bit_width, m);
            bus_write_data_issue_buffer.ingress_data.drive(bus_write_data_issue_fifo.read_data);
            bus_write_data_issue_fifo.read_enable.drive(bus_write_data_issue_buffer.ingress_read_enable);
            bus_write_data_issue_buffer.ingress_data_valid.drive(
                (!bus_write_data_issue_fifo.empty & bus_write_data_issue_buffer.ingress_read_enable)
                .reg_next_with_default(format!("{}_bus_write_data_issue_buffer_read_data_valid", name), false));
            issue_arbiter_primary_issue.bus_write_data.drive(bus_write_data_issue_buffer.egress_data);
            bus_write_data_issue_buffer.egress_read_enable.drive(internal_handshake);

            let bus_write_byte_enable_issue_fifo = Fifo::new(format!("{}_bus_write_byte_enable_issue_fifo", name), issue_fifo_depth_bits, data_byte_width, m);
            bus_write_byte_enable_issue_fifo.write_data.drive(ret.bus_write_byte_enable);
            bus_write_byte_enable_issue_fifo.write_enable.drive(external_handshake);
            let bus_write_byte_enable_issue_buffer = PeekBuffer::new(format!("{}_bus_write_byte_enable_issue_buffer", name), data_byte_width, m);
            bus_write_byte_enable_issue_buffer.ingress_data.drive(bus_write_byte_enable_issue_fifo.read_data);
            bus_write_byte_enable_issue_fifo.read_enable.drive(bus_write_byte_enable_issue_buffer.ingress_read_enable);
            bus_write_byte_enable_issue_buffer.ingress_data_valid.drive(
                (!bus_write_byte_enable_issue_fifo.empty & bus_write_byte_enable_issue_buffer.ingress_read_enable)
                .reg_next_with_default(format!("{}_bus_write_byte_enable_issue_buffer_read_data_valid", name), false));
            issue_arbiter_primary_issue.bus_write_byte_enable.drive(bus_write_byte_enable_issue_buffer.egress_data);
            bus_write_byte_enable_issue_buffer.egress_read_enable.drive(internal_handshake);
