...
impl<'a> PrimaryPort<'a> {
    pub fn connect(&self, replica_port: &ReplicaPort<'a>) {
        ...
        replica_port
            ...
            .drive(self.bus_write_byte_enable);
        self.bus_ready.drive(replica_port.bus_ready);
        self.bus_read_data.drive(replica_port.bus_read_data);
        self.bus_read_data_valid
            .drive(replica_port.bus_read_data_valid);
    }

    pub fn forward(&self, name_prefix: impl Into<String>, m: &'a Module<'a>) -> PrimaryPort<'a> {
        let name_prefix = name_prefix.into();
        let bus_ready = m.input(
            format!("{}_bus_ready", name_prefix),
            self.bus_ready.bit_width(),
        );
        self.bus_ready.drive(bus_ready);
        let bus_read_data = m.input(
            format!("{}_bus_read_data", name_prefix),
            self.bus_read_data.bit_width(),
        );
        self.bus_read_data.drive(bus_read_data);
        let bus_read_data_valid = m.input(
            format!("{}_bus_read_data_valid", name_prefix),
            self.bus_read_data_valid.bit_width(),
        );
        ...
        PrimaryPort {
            ...
            bus_write_data: m.output(
                format!("{}_bus_write_data", name_prefix),
                self.bus_write_data,
            ),
            bus_write_byte_enable: m.output(
                format!("{}_bus_write_byte_enable", name_prefix),
                self.bus_write_byte_enable,
            ),
            bus_ready,
            bus_read_data,
            bus_read_data_valid,
...
impl<'a> ReplicaPort<'a> {
    pub fn forward(&self, name_prefix: impl Into<String>, m: &'a Module<'a>) -> ReplicaPort<'a> {
        let name_prefix = name_prefix.into();
        let bus_enable = m.input(
            format!("{}_bus_enable", &name_prefix),
            self.bus_enable.bit_width(),
        );
        self.bus_enable.drive(bus_enable);
        let bus_addr = m.input(
            format!("{}_bus_addr", &name_prefix),
            self.bus_addr.bit_width(),
        );
        self.bus_addr.drive(bus_addr);
        let bus_write = m.input(
            format!("{}_bus_write", &name_prefix),
            self.bus_write.bit_width(),
        );
        self.bus_write.drive(bus_write);
        let bus_write_data = m.input(
            format!("{}_bus_write_data", &name_prefix),
            self.bus_write_data.bit_width(),
        );
        self.bus_write_data.drive(bus_write_data);
        let bus_write_byte_enable = m.input(
            format!("{}_bus_write_byte_enable", &name_prefix),
            self.bus_write_byte_enable.bit_width(),
        );
...
impl<'a> Crossbar<'a> {
    pub fn new(
        ...
        let primary_issues = (0..num_primaries)
            .map(|i| {
        ...
        issue
            ...
            .drive(issue_arbiter.issue_bus_enable);
        issue.issue_arb_bus_addr.drive(issue_arbiter.issue_bus_addr);
        issue
            .issue_arb_bus_write
            .drive(issue_arbiter.issue_bus_write);
        issue
            .issue_arb_bus_write_data
            .drive(issue_arbiter.issue_bus_write_data);
        issue
            .issue_arb_bus_write_byte_enable
            .drive(issue_arbiter.issue_bus_write_byte_enable);
        issue_arbiter
            .issue_bus_ready
            .drive(issue.issue_arb_bus_ready);
        let replica_issues = (0..num_replicas)
            .map(|i| {
                let issue_replica_issue = &issue.replica_issues[i as usize];
                let name = format!("replica{}", i);
                let ret = ReplicaIssue {
                    bus_enable: m.output(
                        format!("{}_bus_enable", name),
                        issue_replica_issue.bus_enable,
                    ),
                    bus_addr: m.output(format!("{}_bus_addr", name), issue_replica_issue.bus_addr),
                    bus_write: m
                        .output(format!("{}_bus_write", name), issue_replica_issue.bus_write),
                    bus_write_data: m.output(
                        format!("{}_bus_write_data", name),
                        issue_replica_issue.bus_write_data,
                    ),
                    bus_write_byte_enable: m.output(
                        format!("{}_bus_write_byte_enable", name),
                        issue_replica_issue.bus_write_byte_enable,
                    ),
        ...
        let replica_ports = (0..num_primaries)
            .map(|i| {
                ...
                ReplicaPort {
                    ...
                    bus_read_data_valid: m.output(
                        format!("primary{}_bus_read_data_valid", i),
                        bus_read_data_valid,
                    ),
                }
            })
        ...
        if num_primaries > 1 {
            let primary_fifo =
                Fifo::new("primary_fifo", fifo_depth_bits, primary_select_bit_width, m);
            issue
                .issue_arb_bus_primary
                .unwrap()
                .drive(issue_arbiter.issue_bus_primary.unwrap());
            issue.primary_fifo_full.unwrap().drive(primary_fifo.full);
            primary_fifo
                .write_enable
                .drive(issue.primary_fifo_write_enable.unwrap());
            primary_fifo
                .write_data
                .drive(issue.primary_fifo_write_data.unwrap());
            return_arbiter
                .primary_fifo_empty
                .unwrap()
                .drive(primary_fifo.empty);
            return_arbiter
                .primary_fifo_read_data
                .unwrap()
                .drive(primary_fifo.read_data);
            primary_fifo
                .read_enable
                .drive(return_arbiter.primary_fifo_read_enable);
        }

        if num_replicas > 1 {
            let replica_fifo =
                Fifo::new("replica_fifo", fifo_depth_bits, replica_select_bit_width, m);
            issue.replica_fifo_full.unwrap().drive(replica_fifo.full);
            replica_fifo
                .write_enable
                .drive(issue.replica_fifo_write_enable.unwrap());
            replica_fifo
                .write_data
                .drive(issue.replica_fifo_write_data.unwrap());
            ...
            replica_fifo
                .read_enable
                .drive(replica_buffer.ingress_read_enable);
            ...
            return_arbiter
                ...
                .unwrap()
                .drive(replica_buffer.egress_ready);
            return_arbiter
                .replica_buffer_egress_data
                .unwrap()
                .drive(replica_buffer.egress_data);
            replica_buffer
                .egress_read_enable
                .drive(return_arbiter.replica_buffer_egress_read_enable);
        }

        let primary_ports = (0..num_replicas)
            .map(|i| {
                let replica_data_fifo = Fifo::new(
                    format!("replica{}_data_fifo", i),
                    fifo_depth_bits,
                    data_bit_width,
                    m,
                );
                ...
                replica_data_fifo
                    .read_enable
                    .drive(return_arbiter.replica_data_fifo_read_enable_outputs[i as usize]);
                return_arbiter.replica_data_fifo_empty_inputs[i as usize]
                    .drive(replica_data_fifo.empty);
                return_arbiter.replica_data_fifo_read_data_inputs[i as usize]
                    .drive(replica_data_fifo.read_data);
    ...
    fn new(
        ...
        for primary_issue in primary_issues.iter().rev().skip(1) {
            let (
                ...
                new_bus_write,
                new_bus_write_data,
                new_bus_write_byte_enable,
            ) = if_(primary_issue.bus_enable, {
                (
                    m.high(),
                    primary_issue.bus_addr,
                    primary_issue.bus_write,
                    primary_issue.bus_write_data,
                    primary_issue.bus_write_byte_enable,
                )
            })
            .else_({
                (
        ...
        let issue_bus_write_byte_enable =
...
impl<'a> Issue<'a> {
    fn new(
        ...
        let issue_arb_bus_write_byte_enable =
            m.input("issue_arb_bus_write_byte_enable", data_byte_width);

        let primary_fifo_full = if num_primaries > 1 {
            Some(m.input("primary_fifo_full", 1))
        } else {
            None
        };
        let primary_fifo_write_ready = !primary_fifo_full.map(|x| x.into()).unwrap_or(m.low());
        let replica_fifo_full = if num_replicas > 1 {
            Some(m.input("replica_fifo_full", 1))
        } else {
            None
        };
        let replica_fifo_write_ready = !replica_fifo_full.map(|x| x.into()).unwrap_or(m.low());

        let buster_issue_ready =
            issue_arb_bus_write | (primary_fifo_write_ready & replica_fifo_write_ready);
        ...
        for i in 0..num_replicas {
            ...
            let local_replica_select = replica_select
                .map(|x| x.eq(m.lit(i, replica_select_bit_width)))
                .unwrap_or(m.high());
            ...
            replica_issues.push(ReplicaIssue {
                bus_enable: m.output(
                    format!("{}_bus_enable", name),
                    replica_bus_enable & local_replica_select,
                ),
                bus_addr: m.output(
                    format!("{}_bus_addr", name),
                    issue_arb_bus_addr.bits(replica_addr_bit_width - 1, 0),
                ),
                bus_write: m.output(format!("{}_bus_write", name), issue_arb_bus_write),
                bus_write_data: m
                    .output(format!("{}_bus_write_data", name), issue_arb_bus_write_data),
                bus_write_byte_enable: m.output(
                    format!("{}_bus_write_byte_enable", name),
                    issue_arb_bus_write_byte_enable,
                ),
        ...
        let (issue_arb_bus_primary, primary_fifo_write_enable, primary_fifo_write_data) =
            if num_primaries > 1 {
                let issue_arb_bus_primary =
                    m.input("issue_arb_bus_primary", primary_select_bit_width);
                let primary_fifo_write_enable = m.output(
                ...
                let primary_fifo_write_data =
                    m.output("primary_fifo_write_data", issue_arb_bus_primary);
                (
                    Some(issue_arb_bus_primary),
                    Some(primary_fifo_write_enable),
                    Some(primary_fifo_write_data),
                )
            } else {
                (None, None, None)
            };
        ...
        let (replica_fifo_write_enable, replica_fifo_write_data) = if num_replicas > 1 {
            let replica_fifo_write_enable = m.output(
            ...
            let replica_fifo_write_data =
                m.output("replica_fifo_write_data", replica_select.unwrap());
            (
                Some(replica_fifo_write_enable),
                Some(replica_fifo_write_data),
            )
        } else {
            (None, None)
        };

        let issue_arb_bus_ready = m.output(
            "issue_arb_bus_ready",
            buster_issue_ready & replica_bus_ready,
        );
...
impl<'a> ReturnArbiter<'a> {
    fn new(
        ...
        let primary_fifo_empty = if num_primaries > 1 {
            Some(m.input("primary_fifo_empty", 1))
        } else {
            None
        };
        let primary_fifo_read_ready = !primary_fifo_empty.map(|x| x.into()).unwrap_or(m.low());

        let replica_data_fifo_empty_inputs = (0..num_replicas)
            .map(|i| m.input(format!("replica{}_data_fifo_empty", i), 1))
            .collect::<Vec<_>>();
        let replica_data_fifo_read_data_inputs = (0..num_replicas)
            .map(|i| m.input(format!("replica{}_data_fifo_read_data", i), data_bit_width))
            .collect::<Vec<_>>();

        let mut replica_data_fifo_read_ready =
            !replica_data_fifo_empty_inputs[(num_replicas - 1) as usize];
        let mut replica_data =
            replica_data_fifo_read_data_inputs[(num_replicas - 1) as usize].into();
        let (replica_buffer_egress_ready, replica_buffer_egress_data) = if num_replicas > 1 {
            ...
            let replica_buffer_egress_data =
                m.input("replica_buffer_egress_data", replica_select_bit_width);

            let replica_data_fifo_select =
                m.reg("replica_data_fifo_select", replica_select_bit_width);
            replica_data_fifo_select.drive_next(replica_buffer_egress_data);

            for i in (0..num_replicas).rev().skip(1) {
                replica_data_fifo_read_ready = if_(
                    replica_buffer_egress_data.eq(m.lit(i, replica_select_bit_width)),
                    {
                ...
                .else_(replica_data_fifo_read_ready);
                replica_data = if_(
                    replica_data_fifo_select.eq(m.lit(i, replica_select_bit_width)),
                    {
                        let replica_data_fifo_read_data =
                            replica_data_fifo_read_data_inputs[i as usize];
            ...
            (
                Some(replica_buffer_egress_ready),
                Some(replica_buffer_egress_data),
            )
        } else {
            (None, None)
        };

        let fifo_read_enable = primary_fifo_read_ready
            & replica_buffer_egress_ready
                .map(|x| x.into())
                .unwrap_or(m.high())
            & replica_data_fifo_read_ready;
        let primary_fifo_read_enable = m.output("primary_fifo_read_enable", fifo_read_enable);
        let replica_buffer_egress_read_enable =
            m.output("replica_buffer_egress_read_enable", fifo_read_enable);
        let replica_data_fifo_read_enable_outputs = (0..num_replicas)
            .map(|i| {
                m.output(
        ...
        let primary_bus_read_data_outputs = (0..num_primaries)
            .map(|i| m.output(format!("primary{}_bus_read_data", i), replica_data))
            .collect::<Vec<_>>();
        let primary_bus_read_data_valid_outputs = (0..num_primaries)
            .map(|i| {
                let primary_fifo_read_data = primary_fifo_read_data
                    .map(|x| x.eq(m.lit(i as u32, primary_select_bit_width)))
                    .unwrap_or(m.high());
                m.output(
                    format!("primary{}_bus_read_data_valid", i),
                    fifo_read_data_valid & primary_fifo_read_data,
                )
            })
            .collect::<Vec<_>>();
