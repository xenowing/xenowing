...
pub fn generate<'a>(c: &'a Context<'a>) -> &Module<'a> {
    ...
    buster::generate(c, "Sys", 1, 3, 24, 4, 128, 5);
    ...
    m.output("bios_rom_bus_enable", sys.output("replica0_bus_enable"));
    m.output("bios_rom_bus_addr", sys.output("replica0_bus_addr"));
    m.output("bios_rom_bus_write", sys.output("replica0_bus_write"));
    m.output("bios_rom_bus_write_data", sys.output("replica0_bus_write_data"));
    m.output("bios_rom_bus_write_byte_enable", sys.output("replica0_bus_write_byte_enable"));
    sys.drive_input("replica0_bus_ready", m.input("bios_rom_bus_ready", 1));
    sys.drive_input("replica0_bus_read_data", m.input("bios_rom_bus_read_data", 128));
    sys.drive_input("replica0_bus_read_data_valid", m.input("bios_rom_bus_read_data_valid", 1));

    m.output("led_interface_bus_enable", sys.output("replica1_bus_enable"));
    m.output("led_interface_bus_addr", sys.output("replica1_bus_addr"));
    m.output("led_interface_bus_write", sys.output("replica1_bus_write"));
    m.output("led_interface_bus_write_data", sys.output("replica1_bus_write_data"));
    m.output("led_interface_bus_write_byte_enable", sys.output("replica1_bus_write_byte_enable"));
    sys.drive_input("replica1_bus_ready", m.input("led_interface_bus_ready", 1));
    sys.drive_input("replica1_bus_read_data", m.input("led_interface_bus_read_data", 128));
    sys.drive_input("replica1_bus_read_data_valid", m.input("led_interface_bus_read_data_valid", 1));

    m.output("uart_interface_bus_enable", sys.output("replica2_bus_enable"));
    m.output("uart_interface_bus_addr", sys.output("replica2_bus_addr"));
    m.output("uart_interface_bus_write", sys.output("replica2_bus_write"));
    m.output("uart_interface_bus_write_data", sys.output("replica2_bus_write_data"));
    m.output("uart_interface_bus_write_byte_enable", sys.output("replica2_bus_write_byte_enable"));
    sys.drive_input("replica2_bus_ready", m.input("uart_interface_bus_ready", 1));
    sys.drive_input("replica2_bus_read_data", m.input("uart_interface_bus_read_data", 128));
    sys.drive_input("replica2_bus_read_data_valid", m.input("uart_interface_bus_read_data_valid", 1));
